...
// Contact Seller URL
$paymentFormUrl = '#paymentModal';
if (!auth()->check()) {
	if (config('settings.single.guests_can_contact_ads_authors') != '1') {
		$paymentFormUrl = '#quickLogin';
	}
}
?>
			...
			<div class="row">
				<div class="col-md-12">
					@if(session()->has('status'))
						@if(session()->get('status') == 'accredited')
							<div class="alert alert-success alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('approved')}}</strong> {{t('accredited')}}
							</div>
						@endif
						@if(session()->get('status') == 'pending_contingency')
							<div class="alert alert-warning alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('in_process')}}</strong> {{t('pending_contingency')}}
							</div>
						@endif
						@if(session()->get('status') == 'pending_review_manual')
							<div class="alert alert-warning alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('in_process')}}</strong> {{t('pending_review_manual')}}
							</div>
						@endif
						@if(session()->get('status') == 'cc_rejected_bad_filled_card_number')
							<div class="alert alert-danger alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('rejected')}}</strong> {{t('cc_rejected_bad_filled_card_number')}}
							</div>
						@endif
						@if(session()->get('status') == 'cc_rejected_bad_filled_date')
							<div class="alert alert-danger alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('rejected')}}</strong> {{t('cc_rejected_bad_filled_date')}}
							</div>
						@endif
						@if(session()->get('status') == 'cc_rejected_bad_filled_other')
							<div class="alert alert-danger alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('rejected')}}</strong> {{t('cc_rejected_bad_filled_other')}}
							</div>
						@endif
						@if(session()->get('status') == 'cc_rejected_bad_filled_security_code')
							<div class="alert alert-danger alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('rejected')}}</strong> {{t('cc_rejected_bad_filled_security_code')}}
							</div>
						@endif
						@if(session()->get('status') == 'cc_rejected_blacklist')
							<div class="alert alert-danger alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('rejected')}}</strong> {{t('cc_rejected_blacklist')}}
							</div>
						@endif
						@if(session()->get('status') == 'cc_rejected_call_for_authorize')
							<div class="alert alert-danger alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('rejected')}}</strong> {{t('cc_rejected_call_for_authorize')}}
							</div>
						@endif
						@if(session()->get('status') == 'cc_rejected_card_disabled')
							<div class="alert alert-danger alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('rejected')}}</strong> {{t('cc_rejected_card_disabled')}}
							</div>
						@endif
						@if(session()->get('status') == 'cc_rejected_card_error')
							<div class="alert alert-danger alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('rejected')}}</strong> {{t('cc_rejected_card_error')}}
							</div>
						@endif
						@if(session()->get('status') == 'cc_rejected_duplicated_payment')
							<div class="alert alert-danger alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('rejected')}}</strong> {{t('cc_rejected_duplicated_payment')}}
							</div>
						@endif
						@if(session()->get('status') == 'cc_rejected_high_risk')
							<div class="alert alert-danger alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('rejected')}}</strong> {{t('cc_rejected_high_risk')}}
							</div>
						@endif
						@if(session()->get('status') == 'cc_rejected_insufficient_amount')
							<div class="alert alert-danger alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('rejected')}}</strong> {{t('cc_rejected_insufficient_amount')}}
							</div>
						@endif
						@if(session()->get('status') == 'cc_rejected_invalid_installments')
							<div class="alert alert-danger alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('rejected')}}</strong> {{t('cc_rejected_invalid_installments')}}
							</div>
						@endif
						@if(session()->get('status') == 'cc_rejected_max_attempts')
							<div class="alert alert-danger alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('rejected')}}</strong> {{t('cc_rejected_max_attempts')}}
							</div>
						@endif
						@if(session()->get('status') == 'cc_rejected_other_reason')
							<div class="alert alert-danger alert-dismissible fade-in">
								<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
								<strong>{{t('rejected')}}</strong> {{t('cc_rejected_other_reason')}}
							</div>
						@endif
					@endif
						...
						<div class="ads-details">
										...
										<div class="ads-details-info col-md-12 col-sm-12 col-xs-12 enable-long-words from-wysiwyg">
											...
											<div class="row detail-line-action text-center">
													...
													@if (auth()->check())
														...
														@else
															@if ($post->email != '')
																<a onclick="setPaymentForm()" data-toggle="modal" href="{{ $paymentFormUrl }}">
							...
							<div class="content-footer text-left">
								@if (auth()->check())
									...
									@else
										@if ($post->email != '')
											<a onclick="setPaymentForm()" class="btn btn-default" data-toggle="modal" href="{{ $paymentFormUrl }}"><i class="icon-mail-2"></i> {{ t('Send a message') }} </a>
										@endif
									@endif
								@else
									@if ($post->email != '')
							...
							<div class="card-content">
								...
								<div class="ev-action" {!! $evActionStyle !!}>
									@if (auth()->check())
										...
										@else
											@if ($post->email != '')
												<a onclick="setPaymentForm()" href="{{ $paymentFormUrl }}" data-toggle="modal" class="btn btn-default btn-block">
